"""
Dataset loading and memory profiling engine for RoadSafety_ML.
Handles raw, preprocessed, and synthetic dataset loading with type optimization and memory inspection.
"""
...
from app.data.validators import DataValidationError

logger = logging.getLogger(__name__)


def generate_synthetic_dataset(output_path: Union[str, Path], nrows: int = 10000) -> Path:
    """
    Generates a portable, realistic synthetic US traffic accident dataset
    conforming exactly to the schema of the real US Accidents dataset.
    """
    path = Path(output_path)
    logger.info(f"Generating portable synthetic dataset of {nrows} rows at {path}...")
    ...
    data = {
        ...
        "Wind_Direction": np.random.choice(
            ["Calm", "N", "S", "E", "W", "NE", "NW", "SE", "SW", "VAR", "NNE", "NNW", "SSE", "SSW", "ENE", "WNW", "ESE", "WSW"],
            size=nrows
        ),
        ...
        "Weather_Condition": np.random.choice(
            ["Clear", "Fair", "Mostly Cloudy", "Partly Cloudy", "Cloudy", "Overcast", "Light Rain", "Rain", "Heavy Rain", "Fog"],
            size=nrows
        ),
    ...
    # Realistic missing values insertion
    ...
    logger.info(f"Synthetic dataset saved: {path} ({nrows} rows).")
    return path


def resolve_dataset_path(dataset_type: str = "original") -> Path:
    """Resolves and returns the path for the requested dataset type."""
    if dataset_type == "preprocessed":
        if PROCESSED_DATA_PATH.exists():
            return PROCESSED_DATA_PATH
        # If preprocessed file not yet generated, fall back to sample
        logger.warning(f"Preprocessed dataset not found at {PROCESSED_DATA_PATH}. Falling back to raw sample.")
    ...
    # Generate sample if neither exists
    return generate_synthetic_dataset(RAW_SAMPLE_PATH, nrows=10000)


def load_dataset(
    dataset_type: str = "original",
    path: Optional[Union[str, Path]] = None,
    nrows: Optional[int] = 100000,
    optimize_memory: bool = False
) -> pd.DataFrame:
    """
    Loads traffic accident dataset with automatic fallback, row capping, and optional memory downcasting.
    """
    if path is None:
        target_path = resolve_dataset_path(dataset_type)
    else:
        target_path = Path(path)
        if not target_path.exists():
            target_path = resolve_dataset_path(dataset_type)

    logger.info(f"Loading dataset from: {target_path} (limit={nrows} rows)")
...
def get_data_summary(dataset_type: str = "original", nrows: Optional[int] = 100000) -> Dict[str, Any]:
    """
    Calculates detailed memory profiling, schema analysis, missing statistics,
    and preview rows for web studio presentation.
    """
    target_path = resolve_dataset_path(dataset_type)
    df = load_dataset(path=target_path, nrows=nrows)

    # Convert preview NaN to None for clean JSON serialization
    ...
    categorical_cols = list(df.select_dtypes(include=["object", "category", "string", "str"]).columns)
    ...
    for col in df.columns:
        ...
        missing_pct = round((missing_cnt / df.shape[0]) * 100.0, 2) if df.shape[0] > 0 else 0.0
